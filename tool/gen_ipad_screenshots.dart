...
/// Renders App Store 13" iPad marketing screenshots for Tabiu. Run with:
///   flutter test tool/gen_ipad_screenshots.dart
///
/// App Store Connect's "13-inch iPad" display slot requires 2048x2732 (or
/// 2064x2752). We reuse the same raw phone captures, centred as a device
/// card on the deep-violet "Mor Parti" background — a portrait marketing
/// frame Apple accepts for the iPad slot without a real iPad build.
///
/// Source screenshots live in store/screenshots/raw/ and outputs are written
/// to store/screenshots/ipad/.
...
void main() {
  test('generate iPad store screenshots', () async {
    await _loadFredoka();
    Directory('store/screenshots/ipad').createSync(recursive: true);
    for (var i = 0; i < _shots.length; i++) {
      final s = _shots[i];
      final shot = await _decode('store/screenshots/raw/${s[0]}');
      final out = 'store/screenshots/ipad/${(i + 1).toString().padLeft(2, '0')}_'
          '${s[0].substring(2)}';
...
Future<void> _writeFrame(String path,
  ...
  final caption = ui.ParagraphBuilder(ui.ParagraphStyle(
    ...
    ..pushStyle(ui.TextStyle(
      ...
      shadows: const [
        Shadow(color: Color(0x66000000), blurRadius: 20, offset: Offset(0, 7)),
      ],
    ))
    ..addText('$line1\n$line2');
  final para = caption.build()
    ..layout(const ui.ParagraphConstraints(width: _w - 240));
  canvas.drawParagraph(para, const Offset(120, 190));

  // Pink→magenta accent bar under the caption (echoes the in-game card divider).
  final barY = 190 + para.height + 46;
  final barRect =
      Rect.fromCenter(center: Offset(_w / 2, barY), width: 156, height: 14);
  canvas.drawRRect(
  ...
  final rr = RRect.fromRectAndRadius(dst, const Radius.circular(52));

  // Soft drop shadow.
  canvas.drawRRect(
    rr.shift(const Offset(0, 30)),
    Paint()
